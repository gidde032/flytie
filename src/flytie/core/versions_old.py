...
Phase 3 commands (`versions`, `view --version`, `diff`, `restore`) all sit on
top of these helpers so the CLI layer can stay thin.
"""
...
def list_versions(session: Session, name: str) -> list[PatternVersionDTO]:
    """Return all versions of a pattern in ascending order."""
...
def get_version(session: Session, name: str, version_number: int) -> PatternVersionDTO:
    """Return a single historical version of a pattern."""
    pattern = get_pattern(session, name)
    stmt = (
        select(PatternVersion)
        .options(
...
def diff_versions(
    ...
    """Return both version DTOs plus a unified diff of their string fields."""
...
def restore_version(session: Session, name: str, version_number: int) -> PatternVersionDTO:
    """Append a new version copied verbatim from the requested historical version.

    Non-destructive: the old version still exists; the restore is simply a new
    version that happens to share the content of an older one.
    """
    pattern = get_pattern(session, name)
    ...
    payload = PatternInput(
        ...
        # Restore should not touch tags/species — they attach to the pattern,
        # not the version.
        tags=None,
        species=None,
    )
